  ...
  return (
    <main className="h-full bg-gradient-to-b from-contrast from-90% to-contrast-alt space-y-3 lg:px-40 lg:py-10 md:px-20 py-8 px-10">
      <div className="w-full space-y-3">
        ...
        <Input type="text" labelText="Product details (optional)" placeholder="Additional information (name, description, etc.)" onChange={e => { setAdditionalInfo(e.target.value); }} value={additionalInfo} />
      </div>
      <div className="flex">
        <div className="w-1/2 space-y-2">
          ...
          <div className={`${trackPrice ? "opacity-100" : "opacity-50"} flex items-center`}>
            <Input labelText="Notify me when the product goes below:" step={10} min={0} max={100000000} type="number" placeholder="1000 €, £, $, ₩, ¥..." onChange={e => { setTrackPriceThreshold(e.target.value); }} value={trackPriceThreshold} disabled={!trackPrice} />
          </div>
        </div>
